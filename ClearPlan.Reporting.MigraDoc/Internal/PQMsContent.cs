...
namespace ClearPlan.Reporting.MigraDoc.Internal
{
    internal class PQMsContent
    {
        ...
        private void AddPQMRows(Table table, ReportPQMs PQMs, ReportPlanningItem reportPlanningItem, ReportPatient reportPatient)
        {
            ...
            string userLogPath;
            string patientLogId = string.IsNullOrWhiteSpace(reportPatient.LogPatientId) ? reportPatient.Id : reportPatient.LogPatientId;
            string userLogId = string.IsNullOrWhiteSpace(reportPatient.LogUserId) ? reportPatient.userId : reportPatient.LogUserId;
            string filename = string.Format("{0}_{1}_{2}.csv", MakeFilenameValid(patientLogId), MakeFilenameValid(reportPlanningItem.Id.Replace(@"\", "_")), MakeFilenameValid(reportPlanningItem.ConstraintPath));
            ...
            userStatsList.Add(MakeFilenameValid(patientLogId));
