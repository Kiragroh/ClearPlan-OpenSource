...
- Keep `links.feedbackUrl` in `settings.json` pointed to the issue page. File-system paths belong in `settings.ini`.
...
- Plan type or fractionation.
- Relevant structure names or target IDs.
- Whether the issue came from the PQM table, the plan checks, export, or the UI.
- A screenshot or exported CSV/PDF if available.
