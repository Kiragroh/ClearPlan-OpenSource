...
#pragma warning disable 0436
    ...
    public class MeshBuilder
    {
        ...
        /// <param name="startAngle">The start arc angle in radians.</param>
            ...
            int thetaDiv = 360, IList<double> textureValues = null)
        {
            ...
            var circle = GetCircle(thetaDiv);
            //var semicircle = GetCircleSegment(thetaDiv, totalAngle, startAngle);
            int n = section.Count;
            int index0 = this.positions.Count;
            if (arcDirection == "Clockwise")  //CW
            {
                for (int i = Convert.ToInt32(startAngle)-1; i < arcLength ; i++)
                {
                    Console.WriteLine("CW: i = " + i);
                    var w = (v * circle[i].X) + (u * circle[i].Y);
                    for (int j = 0; j < n; j++)
                    {
                ...
                for (int i = Convert.ToInt32(startAngle)-1; i < arcLength; i++)
                {
                    Console.WriteLine("2nd loop:  CW: i = " + i);
                    var ii = (i + 1) % thetaDiv;
            ...
            if (arcDirection == "CounterClockwise")  //CCW
            {
                for (int i = Convert.ToInt32(startAngle-arcLength)-1; i < Convert.ToInt32(arcLength); i++)
                {
                    Console.WriteLine("CCW: i = " + i);
                    var w = (v * circle[i].X) + (u * circle[i].Y);
                    for (int j = 0; j < n; j++)
                    {
                        var q1 = origin + (axis * section[j].Y) + (w * section[j].X);
                ...
                for (int i = Convert.ToInt32(startAngle - arcLength)-1; i < Convert.ToInt32(arcLength); i++)
                {
                    Console.WriteLine("2nd loop:  CCW: i = " + i);
                    var ii = (i + 1) % thetaDiv;
                    for (int j = 0; j + 1 < sectionIndices.Count; j += 2)
                    {
                        var j0 = sectionIndices[j];
        ...
        public void AddTorus(double torusDiameter, double tubeDiameter, int thetaDiv = 36, int phiDiv = 24)
        {
            ...
            // If the second Diameter is zero, we can't build out torus
                ...
                throw new ClearPlanException("Torus must have a Diameter bigger than 0");
