...
namespace VMS.TPS
{
    public class Script : ScriptBase
    {
        ...
        public override void Run(PluginScriptContext context)
        {
            ...
            mainWindow.MinWidth = ReviewWindowSizePolicy.MinimumWidth;
            mainWindow.MinHeight = ReviewWindowSizePolicy.MinimumHeight;
            mainWindow.HorizontalContentAlignment =
                HorizontalAlignment.Stretch;
            mainWindow.VerticalContentAlignment =
        ...
        private static void ShowVersionNotesIfNeeded(PluginScriptContext context, string scriptVersion, ClearPlanSettings settings)
        {
            ...
            string userKey = PrivacyHelper.FormatUserForLog(context.CurrentUser, settings);
        ...
        private void LogUserActivity(PlanningItem selectedPlanningItem, string scriptVersion, User user, Patient patient, ClearPlanSettings settings)
        {
            ...
            try
            {
                ...
                string userId = PrivacyHelper.FormatUserForLog(user, settings);
                string patientId = PrivacyHelper.FormatPatientIdForLog(patient, settings);
