...
namespace ClearPlan
{
    public partial class PlanSelectView : UserControl
    {
        ...
        private void RunButton_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            TryLogPlanSelection(selectedItem);
        ...
        private void TryLogPlanSelection(
            PlanSelectDetailViewModel selectedItem)
        {
            try
            {
            ...
            catch (Exception exception)
            {
                try
                {
                    new CustomLog("PlanSelectionErrors.log").Error(
                        exception,
                        "Plan selection logging was skipped.");
                }
                catch
                {
                    // Optional logging must never close the clinical UI.
                }
            }
        }
        ...
        private void LogPlanSelection(PlanSelectDetailViewModel selectedItem)
        {
            ...
            string userLogPath = settings.GetUsageLogFile();
            ...
            string userId = PrivacyHelper.FormatUserForLog(_psvm.User, settings);
            string patientId = PrivacyHelper.FormatPatientIdForLog(_psvm.Patient, settings);
